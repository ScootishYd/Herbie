...
<body class="flex bg-neutral-100 m-16 gap-6 font-roboto">
                    ...
                    <table class="table-auto text-sm w-full text-center border border-1 border-collapse">
                        ...
                        <tbody>
                            <tr>
                                ...
                                <td>{{ $stok->satuan }}</td>
                                <td>
                                    <p class="px-1 bg-red-600 rounded text-white">{{ $stok->keterangan }}</p>
                                </td>
                                <td>
                                    <div class="flex justify-center gap-1">
                                        <button id="edit-stok-modal"
                                            data-barang="{{ $stok->nama_barang }}"
                                            data-persediaan="{{ $stok->persediaan }}"
                                            data-satuan="{{ $stok->satuan }}"
                                            data-keterangan="{{ $stok->keterangan }}"
                                            onclick="openEditStokModal(this)"
                                            class="mx-1 px-2 outline outline-1 outline-yellow-400 rounded-md text-yellow-400 hover:bg-neutral-200 hover:transition-all hover:duration-300">
                                            Edit</button>
                                        <button class="text-sm text-white px-2 py-1 bg-red-600 hover:bg-red-500 rounded-md">
                                            Delete
                                        </button>
                                    </div>
                                </td>
    ...
    <script>
        ...
        function openEditStokModal(button) {
            const barang = button.getAttribute('data-barang');
            const persediaan = button.getAttribute('data-persediaan');
            const satuan = button.getAttribute('data-satuan');
            const keterangan = button.getAttribute('data-keterangan');

            document.getElementById('namaBarang').value = barang;
